...
bool Scene_CommandActorList(GlobalContext* globalCtx, Ship::SceneCommand* cmd) {
    ...
    {
        if (sceneActorOverrides.find(globalCtx->sceneNum) != sceneActorOverrides.end() &&
                        sceneActorOverrides.at(globalCtx->sceneNum).find(globalCtx->roomCtx.curRoom.num) != sceneActorOverrides.at(globalCtx->sceneNum).end()) {
        ...
        SPDLOG_INFO("Scene: 0x{0:x}, Room: 0x{1:x}, Setup: 0x{2:x}", (uint16_t)globalCtx->sceneNum, (uint16_t)globalCtx->roomCtx.curRoom.num, (uint32_t)gSaveContext.sceneSetupIndex);
...
bool Scene_CommandEntranceList(GlobalContext* globalCtx, Ship::SceneCommand* cmd)
{
    ...
    else
    {
        ...
        SPDLOG_INFO("Scene: 0x{0:x}", (uint16_t)globalCtx->sceneNum);
        ...
        for (int i = 0; i < otrEntrance->entrances.size(); i++)
        {
            ...
            i, globalCtx->setupEntranceList[i].spawn, globalCtx->setupEntranceList[i].room);
        }
...
bool Scene_CommandTransitionActorList(GlobalContext* globalCtx, Ship::SceneCommand* cmd) {
    ...
    if (sceneTransitionActorOverrides.find(globalCtx->sceneNum) != sceneTransitionActorOverrides.end()/* && sceneTransitionLoadFlags.find(globalCtx->sceneNum) == sceneTransitionLoadFlags.end()*/) {
        //sceneTransitionLoadFlags.insert(std::make_pair(globalCtx->sceneNum,true));
        auto& roomOverrides = sceneTransitionActorOverrides.at(globalCtx->sceneNum);
        for (auto& [setup, index, entry] : roomOverrides) {
            if (setup == -1 || setup == gSaveContext.sceneSetupIndex) {
                if (index == -1) {
                    cmdActor->entries.push_back(entry);
