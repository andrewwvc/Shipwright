...
u16 ElfMessage_GetSariaText(GlobalContext* globalCtx) {
    ...
    if (!LINK_IS_ADULT) {
        if (Actor_FindNearby(globalCtx, &player->actor, ACTOR_EN_SA, 4, 800.0f) == NULL) {
            if (globalCtx->sceneNum == SCENE_SPOT04)//Kokiri Village
                return SariaMsg;
            else if (globalCtx->sceneNum == SCENE_LINK_HOME) {//Link's House
                gSaveContext.infTable[27] |= 1;
                return SariaMsg+1;
            }
            else if (globalCtx->sceneNum == SCENE_KOKIRI_SHOP)
                return SariaMsg+2;
            else if (globalCtx->sceneNum == SCENE_KOKIRI_HOME3)
                return SariaMsg+3;
            else if (globalCtx->sceneNum == SCENE_KOKIRI_HOME4)
                return SariaMsg+4;
            else if (globalCtx->sceneNum == SCENE_KOKIRI_HOME5)
                return SariaMsg+5;
            else if (globalCtx->sceneNum == SCENE_KOKIRI_HOME)
                return SariaMsg+6;
            else if (globalCtx->sceneNum == SCENE_MARKET_DAY)
                return SariaMsg+7;
            else if (globalCtx->sceneNum == SCENE_SPOT06)
                return SariaMsg+8;
            else if (globalCtx->sceneNum == SCENE_SPOT02 || globalCtx->sceneNum == SCENE_HUT)
                return SariaMsg+9;
            else if (globalCtx->sceneNum == SCENE_SPOT07)
                return SariaMsg+10;
            else if (globalCtx->sceneNum == SCENE_SPOT15)
                return SariaMsg+11;
            else if (globalCtx->sceneNum == SCENE_SPOT20 || globalCtx->sceneNum == SCENE_SOUKO)
                return SariaMsg+12;
