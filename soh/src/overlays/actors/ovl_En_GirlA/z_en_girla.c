...
s32 EnGirlA_CanBuy_HeartPiece(GlobalContext* globalCtx, EnGirlA* this);
...
void EnGirlA_ItemGive_PieceOfHeart(GlobalContext* globalCtx, EnGirlA* this);
...
void EnGirlA_InitItem(EnGirlA* this, GlobalContext* globalCtx) {
    ...
    if (!gSaveContext.n64ddFlag || !Randomizer_GetSettingValue(RSK_SHOPSANITY)) {
        ...
        if (Object_IsLoaded(&globalCtx->objectCtx, shopItemEntries[params].objID) && (params != SI_SOLD_OUT && globalCtx->sceneNum == SCENE_KOKIRI_SHOP)) {
            this->objBankIndex = Object_GetIndex(&globalCtx->objectCtx, shopItemEntries[params].objID);
        } else {
            this->objBankIndex = Object_Spawn(&globalCtx->objectCtx, shopItemEntries[params].objID);
        }
    } else {
