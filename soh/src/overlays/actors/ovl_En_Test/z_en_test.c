...
void EnTest_WalkAndBlockElite(EnTest* this, PlayState* play) {
    ...
    if (!EnTest_ReactToProjectile(play, this)) {
        ...
        if (120.0f < this->actor.xzDistToPlayer && this->actor.xzDistToPlayer < 220.0f && player->meleeWeaponState != 0) {
            EnTest_SetupThrust(this);
            return;
        }
...
void func_80860F84(EnTest* this, PlayState* play) {
    ...
    if (!EnTest_ReactToProjectile(play, this)) {
        ...
        if ((yawDiff > 0x6800) || yawDiff == SHRT_MIN || (this->timer == 0)) {
            EnTest_ChooseAction(this, play);
...
void EnTest_SlashDown(EnTest* this, PlayState* play) {
    ...
    if ((s32)this->skelAnime.curFrame < 7) {
        Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 1, 0x400, 0);
        this->actor.world.rot.y = this->actor.shape.rot.y;
    }
    ...
    if (SkelAnime_Update(&this->skelAnime)) {
        Player* player = GET_PLAYER(play);
        if ((this->actor.xzDistToPlayer > 60.0f) || player->invincibilityTimer != 0 || Rand_ZeroOne() > 0.8f) {
...
void EnTest_FlinchFront(EnTest* this, PlayState* play) {
    ...
    if (player->meleeWeaponState != 0 && (player->meleeWeaponAnimation != PLAYER_MWA_JUMPSLASH_START && player->meleeWeaponAnimation != PLAYER_MWA_JUMPSLASH_FINISH)) {
        if ((this->actor.bgCheckFlags & 8) && ((ABS((s16)(this->actor.wallYaw - this->actor.shape.rot.y)) < 0x38A4) &&
                                               (this->actor.xzDistToPlayer < 80.0f))) {
            EnTest_SetupJumpUp(this);
...
void EnTest_UpdateDamage(EnTest* this, PlayState* play) {
    Player* player = GET_PLAYER(play);
    s32 bounced = 0;
    s16 isStunning = (this->actor.colChkInfo.damageEffect == STALFOS_DMGEFF_STUN) ||
...
void EnTest_UpdateAI(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.xzDistToPlayer < 110.0f || this->attackFlag != 0) {
        if (this->attackFlag == 0 && player->meleeWeaponState != 0) {
            for (s16 ii = 0; ii < finalElem; ii++){
        ...
        this->attackFlag = player->meleeWeaponState;
    }
}
...
void EnTest_Update(Actor* thisx, PlayState* play) {
    ...
    if (!(this->swordCollider.base.atFlags & AT_BOUNCED)) {
        ...
        if (this->actionFunc == EnTest_SpinAttack)
            Player_SetShieldRecovery(play, 10);//NOTE: Make this dynamic, so that the sooner the block, the fewer recovery frames induced
