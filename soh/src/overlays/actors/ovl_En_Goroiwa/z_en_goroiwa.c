...
s32 EnGoroiwa_isNormal(EnGoroiwa* this) {
    return ((this->actor.params & 0x7000) == 0x0000);
}
...
u8 EnGoroiwa_GetPathNo(EnGoroiwa* this) {
    return this->actor.params & 0xFF;
}

void EnGoroiwa_SetPath(EnGoroiwa* this, u8 path) {
    this->actor.params &= 0xFF00;//Clear lower bits
    this->actor.params |= (u16)path;
}

void EnGoroiwa_FaceNextWaypoint(EnGoroiwa* this, PlayState* play) {
    Path* path;
    path = EnGoroiwa_GetPath(this,play);
...
void EnGoroiwa_GetPrevWaypointDiff(EnGoroiwa* this, PlayState* play, Vec3f* dest) {
    ...
    dest->y = currentPointPos->y - prevPointPos->y;
    dest->z = currentPointPos->z - prevPointPos->z;
}
...
void EnGoroiwa_AlterPathOnDrop(EnGoroiwa* this, PlayState* play) {
    if (EnGoroiwa_GetPathNo(this) != CUSTOM_PATH_NO) {
        EnGoroiwa_SpawnFragments(this, play);
        EnGoroiwa_SetPath(this,CUSTOM_PATH_NO);
        EnGoroiwa__Special_PointsCustomEnd[0].x = this->actor.world.pos.x;
        EnGoroiwa__Special_PointsCustomEnd[0].y = this->actor.world.pos.y;
        EnGoroiwa__Special_PointsCustomEnd[0].z = this->actor.world.pos.z;
        EnGoroiwa_InitPath(this,play);
        EnGoroiwa_FaceNextWaypoint(this, play);
        EnGoroiwa_SetupRoll(this);
    }
...
void EnGoroiwa_Update(Actor* thisx, PlayState* play) {
    ...
    if (!(player->stateFlags1 & 0x300000C0)) {
        ...
        if (!EnGoroiwa_isNormal(this) && (this->actor.bgCheckFlags & 1)) {
