...
s32 func_80AF5DFC(EnSa* this, GlobalContext* globalCtx) {
    ...
    if (globalCtx->sceneNum == SCENE_SPOT05 && (gSaveContext.eventChkInf[4] & 1) && (!CHECK_QUEST_ITEM(QUEST_SONG_SARIA) || (!(getDayOfCycle() % 3 == 2) && !(gSaveContext.infTable[27]&1)))) {
    ...
    if (globalCtx->sceneNum == SCENE_LINK_HOME && !LINK_IS_ADULT && (gSaveContext.infTable[27]&1)) {
            return 1;
    }
    return 0;
}
