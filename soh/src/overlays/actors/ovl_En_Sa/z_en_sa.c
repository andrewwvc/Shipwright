...
s32 func_80AF5DFC(EnSa* this, PlayState* play) {
    ...
    if (play->sceneNum == SCENE_SACRED_FOREST_MEADOW && (Flags_GetEventChkInf(EVENTCHKINF_OBTAINED_ZELDAS_LETTER)) && (!CHECK_QUEST_ITEM(QUEST_SONG_SARIA) || (!isSariasDayHome() && !isSariaAtLinksHouse()))) {
        if (IS_RANDO) {
            return 5;
        }
    ...
    if (play->sceneNum == SCENE_LINKS_HOUSE && !LINK_IS_ADULT && isSariaAtLinksHouse()) {
            return 1;
    }
    return 0;
}
