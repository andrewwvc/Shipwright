...
static s16 sGetItemIds[6] = {
    GI_RUPEE_GOLD, GI_WALLET_ADULT, GI_HEART_PIECE, GI_WALLET_GIANT, GI_STONE_OF_AGONY, GI_HEART_PIECE,
};
    ...
    if (this->actor.params == 0) {
        if ((msgState == TEXT_STATE_EVENT) && msgShouldAdvance) {
            ...
            if (this->actor.textId == CursedFamilyMsg+4) {
                if (play->msgCtx.choiceIndex == 0 && gSaveContext.subTimerState != 0) {
