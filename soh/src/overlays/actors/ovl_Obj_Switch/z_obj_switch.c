...
s32 ObjSwitch_EyeIsHit(ObjSwitch* this, GlobalContext* globalCtx) {
        ...
        if (collidingActor != NULL) {
            ...
            if (ABS(yawDiff) > 0x5000) {
                if (collidingActor->params == 4) { //Ice Arrow
                    ...
                    ObjSwitch_SpawnIce(this,globalCtx);
                    return 0;
                }
                return 1;
            }
...
void ObjSwitch_EyeOpen(ObjSwitch* this, GlobalContext* globalCtx) {
    if (ObjSwitch_EyeIsHit(this,globalCtx) || (this->dyna.actor.params >> 7 & 1)) {
