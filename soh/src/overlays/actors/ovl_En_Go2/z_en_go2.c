...
//Actor* explosive = Actor_FindNearby(globalCtx, &this->actor, -1, ACTORCAT_EXPLOSIVE, 1000.0f);
void EnGo2_Alter_BigRoller_Path(EnGo2* this, GlobalContext* globalCtx) {
    ...
    if (oldPath == &EnGo2_Special_Path[0]) {
        if ((this->waypoint == 2)) {
            //s32 numBombs = Actor_FindNumberOf(globalCtx,&this->actor,-1,ACTORCAT_EXPLOSIVE,2000.0f,NULL,NULL);
            u8 forwardCollision = 0;
            u8 altCollision = 0;
            //Find if player is in paths
...
void EnGo2_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    switch (this->actor.params & GORON_IDENTITY_PARAM) {
        ...
        case GORON_CITY_LINK:
            ...
            break;
        case GORON_CITY_ROLLING_BIG:
            if (LINK_IS_ADULT && ((!LINK_IS_ADULT ^ !!(gSaveContext.goronTimeStatus & (1<<5))) || ((globalCtx->sceneNum == SCENE_SPOT18) && IS_NIGHT)))
                Actor_Kill(&this->actor);
        case GORON_DMT_ROLLING_SMALL:
            this->collider.dim.height = (D_80A4816C[this->actor.params & GORON_IDENTITY_PARAM].height * 0.6f);
            EnGo2_SetupRolling(this, globalCtx);
