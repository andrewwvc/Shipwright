...
u16 func_80A97610(PlayState* play, Actor* thisx) {
    ...
    if (ENKO_TYPE == ENKO_TYPE_CHILD_0 || ENKO_TYPE == ENKO_TYPE_CHILD_2 || ENKO_TYPE == ENKO_TYPE_CHILD_3 ||
        ...
        u16 KokiriMsg = GetTextID("kokiri");
        if (Player_GetMask(play) == PLAYER_MASK_GORON)
            faceReaction = KokiriMsg+36;
        else
...
s32 EnKo_GetForestQuestState2(EnKo* this) {
    ...
    if (CHECK_QUEST_ITEM(QUEST_KOKIRI_EMERALD)) {
        return (Flags_GetEventChkInf(EVENTCHKINF_OBTAINED_ZELDAS_LETTER)) ? (CHECK_QUEST_ITEM(QUEST_SONG_SARIA) ? ENKO_FQS_CHILD_POST : ENKO_FQS_CHILD_SARIA) : ENKO_FQS_CHILD_STONE;
    }
    return ENKO_FQS_CHILD_START;
}
